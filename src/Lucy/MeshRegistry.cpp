...
void lucy::MeshRegistry::Add(Mesh* mesh, const std::string& name, const std::string& filepath, const UTIL_UUID& id) {
	assert(mesh_store.find(id) == mesh_store.end());

	mesh_store[id] = { mesh, name, filepath };
}

const UTIL_UUID& lucy::MeshRegistry::GetMeshID(Mesh* mesh) {
...
lucy::Mesh* lucy::MeshRegistry::GetByID(const UTIL_UUID& id) {
